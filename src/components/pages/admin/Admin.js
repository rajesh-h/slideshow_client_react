...
class Admin extends React.Component {
  ...
  componentDidMount() {
    axios.get(this.base_url + 'api/slides').then((res) =>
      this.setState(
  ...
  //Delete Slide
  delSlide = (id) =>
    axios.delete(this.base_url + `api/slide/${id}`).then((res) =>
      this.setState({
        slides: [...this.state.slides.filter((slide) => slide.id !== id)]
      })
    )
  ...
  editSlide = (id, title, imageurl, description) => {
    ...
    axios
      .patch(this.base_url + `api/slide/${id}`, {
        title: title,
        imageurl: imageurl,
  ...
  render() {
    return (
      <div className="Admin">
        <h3>Manage Slides</h3>
        <p style={{ color: 'red' }}>
          In Real world This page will be accessible only after succesful Authentication
        </p>
        <hr />
