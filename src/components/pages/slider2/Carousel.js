...
class RRCarousel extends Component {
  ...
  componentDidMount() {
    axios.get(this.base_url + 'api/activeslides').then((res) =>
      this.setState(
        {
  ...
  render() {
    return this.state.imagesLoaded ? (
      <div>
        <div>
          <h4>This is generated using React Responsive Carousel Package</h4>
        </div>
        <Carousel infiniteLoop autoPlay swipeable={true}>
          {this.state.activeSlides.map((value, index) => {
            return (
              <div key={index}>
                <img src={this.base_url + value.imageurl} alt="" />
